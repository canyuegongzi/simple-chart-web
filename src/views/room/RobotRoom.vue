...
export default {
    ...
    methods: {
        ...
        async getMessageValue(value) {
            const info ={
                type: "TEXT",
                targetId: this.targetChartObj.targetInfo.friendId+'',
                content: value,
                createTime: new Date().getTime(),
                userId: this.currentInfo.userId+'',
                multiWheel: false,
                otherQuery: {
                    logId: this.answerResponse.logId, // this.answerResponse
                    botSession: this.answerResponse.botSession, // this.answerResponse
                    botId: this.answerResponse.botId, // this.answerResponse
                }
            };
            ...
            try {
                ...
                const messageResult = await this.$post(IMCenterApi.robotMessage.url, info, IMCenterApi.robotMessage.server);
                console.log(messageResult);
                tempMessageInfo.success = true
                this.answerResponse = messageResult.data.answerData;
                const dataObj = {
                    content: messageResult.data.answerData.answer,
                    createTime: new Date().getTime(),
                    friendId: this.currentInfo.userId+'',
                    ...
                    userInfo: {
                        ...
                        userId: this.targetChartObj.targetInfo.friendId+'',
                        userName: this.targetChartObj.targetInfo.friendName,
                    }
        ...
        async loadFriendMessageList() {
            let list = []
            try {
                const res = await this.$get(IMCenterApi.robotMessageList.url, {page: this.page, pageSize: this.pageSize, userId: this.currentInfo.userId, friendId: this.targetChartObj.targetInfo.friendId}, IMCenterApi.robotMessageList.server)
                list = res.data.data
                list.forEach((item) => {
