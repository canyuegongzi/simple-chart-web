<template lang="pug">
    van-pull-refresh(v-model="refreshing" @refresh="onRefresh" style="height: 100%;overflow-y: scroll;overflow-x: hidden;")
        van-list(v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad")
            van-cell(v-for="(item, index) in friendList" :key="index" center @click="clickItem(item)"  :title="item.friendName" size="mini" style="padding: 4px 16px;")
                template(#icon)
                    div(style="display:flex;align-items: center;padding-right: 8px;")
                        van-icon(:name="item.friendIcon" size="32")
</template>
    ...
    export default {
        ...
        methods: {
            ...
            async onRefresh() {
                ...
                if (wsToken && wsToken.token) {
                    await this.getUserFriendList({userId: wsToken.userId, deviceType: 'PC'})
                    this.loading = false;
                    this.finished = true;
